...
class _PageIndicatorWidgetState extends State<PageIndicatorWidget> {
  ...
  void _actionGotoPage(int currentPage) {
    final double page = currentPage.toDouble();
    if (widget.controller != null) {
      if (widget.onChangePage != null) {
        widget.onChangePage!(page);
      } else {
        final int pageNext = page.toInt();
  ...
  Widget build(BuildContext context) {
    final Color colorDot = widget.color ?? Colors.transparent;
    final Color colorDotActive = widget.colorActive ?? Colors.white;
